...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    if (scene == 0)
    {
        ...
        if (shader->GetProgramID())
        {
            ...
            int loc_generator_position = glGetUniformLocation(shader->program, "generator_position");
            glUniform3f(loc_generator_position, generator_position.x, generator_position.y, generator_position.z);
            int loc_deltaTime = glGetUniformLocation(shader->program, "deltaTime");
            glUniform1f(loc_deltaTime, deltaTimeSeconds);
            int loc_offset = glGetUniformLocation(shader->program, "offset");
